...
#포트폴리오 이름
PortfolioName = "동적자산배분전략_ADM"

'''
ADM전략

매월 마지막 거래일에 SPY 및 국제 소형주(SCZ)의 "모멘텀 점수"를 계산한 후, 모멘텀 점수는 각 자산에 대한 1,3,6개월 총 수익률의 평균.

SCZ > SPY 이면 SCZ에 100%투자
SCZ < SPY 이면 SPY에 100%투자

모멘텀 점수가 모두 0보다 크지 않은 경우 장기 미국국채(TLT)및 미국 TIPS(TIP)에 대한 1개월 총 수익률을 계산한 후, 수익률이 더 높은 곳에 투자
...
# 0 1 1-7 * 1-5 python3 /Users/TY/Documents/class101/Dynamic_Asset/Dynamic_Asset_ADM_KR.py 
...
#파일 경로입니다.
static_asset_tym_file_path = "/Users/TY/Documents/class101/Dynamic_Asset/Dynamic_Asset_ADM_KR.json"
try:
...
위험 자산
미국주식      KODEX 미국S&P500TR "379800"  or TIGER 미국 S&P 500 "360750"
국제소형주?   KODEX 선진국 MSCI World "251350" or ARIRANG 글로벌MSCI(합성 H) "189400"


안전자산


장기채권 KODEX 200미국채혼합 "284430"
물가연동채  KOSEF 물가채KIS "430500"
...
asset4 = dict()
asset4['stock_code'] = "430500"
asset4['stock_type'] = "SAFE"         
...
for stock_info in MyPortfolioList:
    ...
    if stock_info['stock_type'] == "RISK":
        ...
        # 1개월 수익률, 3개월 수익률, 6개월 수익률의 평균을 모멘텀 스코어로!!!
        MomentumScore = ((((Now_Price - One_Price) / One_Price)) + (((Now_Price - Three_Price) / Three_Price)) + (((Now_Price - Six_Price) / Six_Price))) / 3.0
    ...
    #안전 자산은 1개월 수익률
        ...
        # 1개월 수익률
        MomentumScore = (((Now_Price - One_Price) / One_Price))
