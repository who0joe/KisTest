...
#투자할 종목! KODEX 레버리지 , TIGER 미국나스닥100레버리지(합성)
TargetStockList = ['122630','418660']
            ...
            if StockInfo['StockCode'] == stock_code:
                    ...
                    #트레일링 스탑 아직 시작 안됨!
                        ...
                        if CurrentPrice >= RevenuePrice:
                            ...
                            if stock_amt == 1:
                                ...
                                msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ")  이평무한매수봇 절반 팔아서 수익확정!!!!  [" + str(stock_revenue_money) + "] 수익 조으다! (현재 [" + str(StockInfo['Round']) + "] 라운드까지 진행되었고 절반 익절 후 트레일링 스탑 시작!!)"
                                print(msg) 
                                line_alert.SendMessage(msg) 
                ...
                if StockInfo['IsReady'] == 'Y' and StockInfo['IsTralling'] =='N':
                    ...
                    if CurrentPrice > StMoney:
                        ...
                        msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ")  현재 50분할된 투자금보다 현재가가 높아서 전략을 제대로 수행 못해요! 투자비중이나 자금을 늘리세요!"
                        print(msg)   
                        line_alert.SendMessage(msg) 
