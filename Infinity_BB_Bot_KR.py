...
#투자할 종목! 삼성전자, LG에너지솔루션, 네이버
TargetStockList = ['005930','373220','035420']
...
if IsMarketOpen == True:
    ...
    for stock_code in TargetStockList:
        ...
        if PickStockInfo == None:
            ...
            msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ") BB모아봇 첫 시작!!!!"
            print(msg) 
            ...
            if StockInfo['StockCode'] == stock_code:
                ...
                if MustHasAmt > stock_amt:
                    ...
                    if GapAmt >= 1:
                        ...
                        #시장가 주문을 넣는다!
                        pprint.pprint(KisKR.MakeBuyMarketOrder(stock_code,GapAmt))

                        msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ")  BB모아봇 60% 비중이 안차서 " + str(GapAmt)+ "개 추가 매수 했어요!"
                        print(msg) 
                        line_alert.SendMessage(msg) 
                ...
                if StockInfo['IsReady'] == 'Y':
                    ...
                    #캔들 데이터를 읽는다
                    df = Common.GetOhlcv("KR",stock_code, 70)
                        ...
                        for DantaData in list(StockInfo['DantaList']):
                            if DantaData['IsBuy'] == 'Y':
                                ...
                                if RevenuePrice <= CurrentPrice: 
                                    ...
                                    #저장된 단타수량을 매도!
                                    pprint.pprint(KisKR.MakeSellMarketOrder(stock_code,DantaData['DantaAmt']))

                                    msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ")  BB모아봇 ID:" + str(DantaData['Id']) + " -> ["+str(DantaData['DantaAmt'])+"]개 단타세계관에서 익절로 팔았어요!!!!"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 
                        ...
                        if IsSell == True and IsDone == False  and IsFull == True:
                            ...
                            for DantaData in list(StockInfo['DantaList']):
                                if DantaData['Id'] == PickId:
                                    ...
                                    msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ")  BB모아봇 ID:" + str(DantaData['Id']) + " -> ["+str(DantaData['DantaAmt'])+"]개 단타세계관에서 손절로 팔았어요!!!!"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 
                        ...
                        if IsBuy == True:
                            ...
                            for DantaData in list(StockInfo['DantaList']):
                                ...
                                if DantaData['IsBuy'] == 'N':
                                    ...
                                    msg = KisKR.GetStockName(stock_code) + "("+ stock_code  + ") BB모아봇 " + str(BuyAmt) + "개 "  +  str(CurrentPrice) + "가격으로 단타 매수 완료!"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 
